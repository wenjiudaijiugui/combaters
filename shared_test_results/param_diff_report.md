...
- **supported**: `par_prior=True` and `par_prior=False`, `mean_only` true or false, optional original-label `ref_batch`, optional `mod` as numeric ndarray, DataFrame/Series-like input, or dummy-coded categorical covariates, factor-like `batch` labels, and NA-aware `values`
